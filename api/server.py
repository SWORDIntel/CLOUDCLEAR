...
app.config["SECRET_KEY"] = os.environ.get(
    "SECRET_KEY", "cloudclear-secret-key-change-me"
)
app.config["MAX_CONTENT_LENGTH"] = 16 * 1024  # 16KB max request size
CORS(app)
socketio = SocketIO(
    app, cors_allowed_origins="*", async_mode="eventlet", ping_timeout=60
)
...
def info():
    ...
    cloud_providers = [
        ...
        {
            "name": "Oracle Cloud",
            "enabled": bool(os.getenv("ORACLE_CLOUD_TENANCY_OCID")),
        },
    ...
    return jsonify(
        {
            ...
            "capabilities": {
                ...
                "enabled_intelligence": sum(
                    1 for s in intelligence_services if s["enabled"]
                ),
            },
            "integrations": {
...
def scan():
    ...
    try:
        ...
        return jsonify(
            {"error": "Internal server error", "message": "Failed to initiate scan"}
        ), 500
...
def run_scan(scan_id, domain):
    ...
    try:
        ...
        socketio.emit(
            "scan_complete",
            {"scan_id": scan_id, "status": "completed", "results": results},
        )
        ...
        with scan_lock:
            if len(active_scans) > 100:
                oldest_scans = sorted(
                    active_scans.items(), key=lambda x: x[1].get("started_at", "")
...
def list_scans():
    ...
    with scan_lock:
        return jsonify(
            {"scans": list(active_scans.values()), "count": len(active_scans)}
        )
...
def handle_subscribe(data):
    ...
    if scan_id:
        ...
        emit(
            "subscribed",
            {"scan_id": scan_id, "timestamp": datetime.utcnow().isoformat()},
        )
...
if __name__ == "__main__":
    ...
    if not in_docker:
        ...
        logger.warning(
            "For local testing, use the CLI/TUI instead: ./cloudclear example.com"
        )
        logger.warning("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("CloudClear API Server v2.0-Enhanced-Cloud")
    logger.info(f"Starting on {host}:{port}")
    logger.info(
        f"Environment: {'Docker' if in_docker else 'Standalone (not recommended)'}"
    )
