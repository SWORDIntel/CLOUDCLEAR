...
class AIDocumentClassifier:
        ...
        if content is None:
            try:
                if file_path.suffix.lower() == ".pdf":
                    content = PDFExtractor.extract_with_cache(file_path)[
                        :2000
                    ]  # First 2K chars
                else:
    ...
    def _generate_overview(self, file_path: Path, content: str) -> str:
        ...
        if doc_info["category"] == "agent_files":
            caps = (
                " AND ".join(doc_info["capabilities"]).upper()
                if doc_info["capabilities"]
                else "SPECIALIZED PROCESSING"
            )
            ...
            system = (
                doc_info["language"] if doc_info["language"] != "GENERAL" else "SYSTEM"
            )
...
class DocumentationStructureAnalyzer:
    ...
    def analyze_structure(self) -> Dict:
        ...
        if self.config_file.exists():
            try:
                with open(self.config_file, "r") as f:
                    ...
                    if (
                        saved_config.get("structure")
                        and abs(
                            self.root_path.stat().st_mtime
                            - saved_config.get("analyzed_at", 0)
                        )
                        < 3600
                    ):
        ...
        ignore_dirs = {
            ...
            "node_modules",
            ".git",
            ".svn",  # Version control and dependencies
            "__pycache__",
        ...
        for item in self.root_path.iterdir():
            if (
                ...
                and item.name.lower() not in ignore_dirs
            ):
    ...
    def generate_role_mappings(self) -> Dict[str, List[str]]:
        ...
        for cat_name, cat_info in self.structure.get("categories", {}).items():
            ...
            for file_name in files[:5]:  # Increased from 3 to 5 files per category
                try:
                    relative_path = (
                        str(Path(cat_path.name) / file_name)
                        if cat_name != "_root"
                        else file_name
                    )
...
class UniversalDocumentationBrowser:
    ...
    def setup_ui(self):
        ...
        content_frame.grid(
            row=1, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(5, 0)
        )
    ...
    def create_toolbar(self, parent):
        ...
        pdf_status = (
            "✓ Available" if PDF_EXTRACTION_AVAILABLE else "✗ Install pdfplumber"
        )
        pdf_color = "green" if PDF_EXTRACTION_AVAILABLE else "red"
        ttk.Label(
            pdf_frame, text=pdf_status, foreground=pdf_color, font=("Arial", 8)
        ).pack()

        # Role-based quick access (only if roles were detected)
        if self.role_mappings:
            role_frame = ttk.LabelFrame(
                toolbar, text="Quick Access by Role", padding="5"
            )
            ...
            dir_frame = ttk.LabelFrame(
                toolbar, text="Directory Navigation", padding="5"
            )
            dir_frame.grid(row=0, column=1, sticky=(tk.W, tk.N), padx=(0, 10))

            ttk.Button(
                dir_frame, text="Change Directory", command=self.change_directory
            ).pack(pady=2)
            ttk.Button(dir_frame, text="Refresh", command=self.refresh_structure).pack(
                pady=2
            )
        ...
        search_entry = ttk.Entry(
            search_frame, textvariable=self.search_var, font=("Arial", 10)
        )
    ...
    def create_navigation_panel(self, parent):
        ...
        nav_frame = ttk.LabelFrame(
            parent, text=f"Documentation Structure ({self.docs_path.name})", padding="5"
        )
    ...
    def create_content_panel(self, parent):
        ...
        content_v_scroll = ttk.Scrollbar(
            viewer_frame, orient=tk.VERTICAL, command=self.content_text.yview
        )
        content_v_scroll.grid(row=0, column=1, sticky=(tk.N, tk.S))
        self.content_text.configure(yscrollcommand=content_v_scroll.set)

        content_h_scroll = ttk.Scrollbar(
            viewer_frame, orient=tk.HORIZONTAL, command=self.content_text.xview
        )
        ...
        ttk.Button(content_toolbar, text="Copy Path", command=self.copy_file_path).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(
            content_toolbar, text="Show in Folder", command=self.show_in_folder
        ).pack(side=tk.LEFT, padx=(0, 5))

        # PDF specific buttons
        self.pdf_extract_btn = ttk.Button(
            content_toolbar,
    ...
    def load_documentation_tree(self):
        ...
        try:
            ...
            self.tree.tag_configure(
                "category", background="lightblue", font=("Arial", 10, "bold")
            )
    ...
    def _find_file_in_category(
        self, category_path: Path, filename: str
    ) -> Optional[Path]:
    ...
    def load_file_content(self, file_path: Path):
        ...
        try:
            ...
            pdf_state = (
                "normal" if (is_pdf and PDF_EXTRACTION_AVAILABLE) else "disabled"
            )
            ...
            if file_path.suffix.lower() == ".pdf":
                ...
                if str(file_path) in self.extracted_pdfs:
                    ...
                    if PDF_EXTRACTION_AVAILABLE:
                        ...
                        self.content_text.insert(
                            tk.END,
                            "Click 'Open External' to view with system PDF viewer.\n\n",
                        )
                    else:
                        self.content_text.insert(
                            tk.END, "PDF text extraction not available.\n"
                        )
                        self.content_text.insert(
                            tk.END, "Install with: pip install pdfplumber\n\n"
                        )
                        self.content_text.insert(
                            tk.END,
                            "Click 'Open External' to view with system PDF viewer.\n\n",
                    ...
                    self.content_text.insert(
                        tk.END,
                        f"File size: {self.format_file_size(file_path.stat().st_size)}\n",
                    )
                ...
                self.content_text.insert(
                    tk.END, f"Type: {file_path.suffix.upper()} file\n"
                )
                self.content_text.insert(
                    tk.END, "Click 'Open External' to view this file.\n\n"
                )
                self.content_text.insert(tk.END, f"File path: {file_path}\n")
                self.content_text.insert(
                    tk.END,
    ...
    def extract_pdf_text(self):
        ...
        if not PDF_EXTRACTION_AVAILABLE:
            messagebox.showerror(
                "PDF Extraction",
                "PDF extraction not available. Install pdfplumber:\npip install pdfplumber",
            )
            return
        ...
        progress_label = ttk.Label(
            progress_window, text="Extracting PDF text...", font=("Arial", 10)
        )
        ...
        def extraction_thread():
            try:
                ...
                extracted_text = PDFExtractor.extract_text(
                    self.current_file, update_progress
                )
                ...
                progress_window.destroy()
                messagebox.showerror(
                    "PDF Extraction Error", f"Failed to extract text: {e}"
                )
    ...
    def show_pdf_info(self):
        ...
        if not PDF_EXTRACTION_AVAILABLE:
            messagebox.showerror(
                "PDF Info",
                "PDF processing not available. Install pdfplumber:\npip install pdfplumber",
            )
            return
    ...
    def refresh_structure(self):
        ...
        try:
            ...
            # Update UI
            project_name = (
                self.docs_path.name.replace("-", " ").replace("_", " ").title()
            )
            self.root.title(f"Universal Documentation Browser - {project_name}")
            ...
            messagebox.showinfo(
                "Refresh", "Documentation structure refreshed successfully!"
            )
    ...
    def show_role_documents(self, role: str):
        ...
        if not docs:
            messagebox.showinfo(
                "Role Documents", f"No documents found for role: {role}"
            )
            return
        ...
        ttk.Label(
            frame, text=f"Recommended documents for {role}:", font=("Arial", 12, "bold")
        ...
        ttk.Button(btn_frame, text="Open Document", command=open_selected).pack(
            side=tk.LEFT
        )
    ...
    def perform_search(self):
        ...
        try:
            ...
            for cat_name, cat_info in self.categories.items():
                ...
                if cat_name == "_root":
                    files_to_search = [
                        self.docs_path / f for f in cat_info.get("files", [])
                    ]
                else:
                ...
                for file_path in files_to_search:
                    ...
                    if search_term.lower() in file_path.name.lower():
                        results.append(
                            (
                                file_path,
                                "filename",
                                f"Found in filename: {file_path.name}",
                            )
                        )
                    ...
                    if file_path.suffix.lower() == ".pdf":
                        ...
                        if PDF_EXTRACTION_AVAILABLE:
                            ...
                            if (
                                pdf_content
                                and search_term.lower() in pdf_content.lower()
                            ):
                                # Find context
                                ...
                                for i, line in enumerate(lines, 1):
                                    if search_term.lower() in line.lower():
                                        ...
                                        results.append(
                                            (file_path, f"PDF line {i}", context)
                                        )
                        ...
                        try:
                            ...
                            for i, line in enumerate(lines, 1):
                                if search_term.lower() in line.lower():
                                    ...
                                    if (
                                        len([r for r in results if r[0] == file_path])
                                        >= 3
    ...
    def show_search_results(self, search_term: str, results):
        ...
        if not results:
            messagebox.showinfo(
                "Search Results", f"No results found for: {search_term}"
            )
        ...
        v_scrollbar = ttk.Scrollbar(
            tree_frame, orient=tk.VERTICAL, command=results_tree.yview
        )
        v_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        results_tree.configure(yscrollcommand=v_scrollbar.set)

        h_scrollbar = ttk.Scrollbar(
            tree_frame, orient=tk.HORIZONTAL, command=results_tree.xview
        )
        ...
        for file_path, location, context in results:
            try:
                ...
                results_tree.insert(
                    "",
                    "end",
                    values=(f"{type_indicator} {relative_path}", location, context),
                )
            except ValueError:
                # Handle files outside the docs path
                results_tree.insert(
                    "", "end", values=(str(file_path), location, context)
                )
        ...
        ttk.Button(btn_frame, text="Open Selected", command=open_result).pack(
            side=tk.LEFT
        )
        ttk.Button(btn_frame, text="Close", command=results_window.destroy).pack(
            side=tk.RIGHT
        )
    ...
    def open_external(self):
        ...
        try:
            if sys.platform.startswith("darwin"):  # macOS
                ...
                messagebox.showinfo(
                    "Open External", f"Please open manually: {self.current_file}"
                )
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Universal Documentation Browser with PDF extraction"
    )
    parser.add_argument(
        "directory",
        nargs="?",
    ...
    print("Universal Documentation Browser with PDF extraction")
    print(f"Analyzing documentation at: {docs_path}")
    print(
        f"PDF extraction: {'✓ Available' if PDF_EXTRACTION_AVAILABLE else '✗ Install pdfplumber'}"
    )
